...
LRESULT __stdcall wndProc(HWND window, UINT msg, WPARAM wParam, LPARAM lParam) noexcept
{
	[[maybe_unused]] static const auto initGlobal = [](HWND window) noexcept
	{
		...
		config = std::make_unique<Config>("MEOWENGINE");
		...
		welcomeMsg << "Configs are stored in Documents/MeowEngine/ directory.\n";
		welcomeMsg << "MeowEngine tries to load a config named \"default\" on start-up,\nand it appears that it was ";
		welcomeMsg << (loaded ? "loaded successfuly." : "not found.");

		interfaces->gameUI->createCommandMsgBox("Welcome to MeowEngine", welcomeMsg.str().c_str());
